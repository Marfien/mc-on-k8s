...
public class AgonesClient implements AutoCloseable {
    ...
    public void setReserved(Duration duration, VoidStreamObserver observer) {
        this.sdk.reserve(dev.agones.sdk.Duration.newBuilder().setSeconds(duration.toSeconds()).build(), observer.toSdk());
    }
    ...
    public void updateList(ListStore list, FieldMask updateMask, StreamObserver<ListStore> observer) {
        this.alphaSdk.updateList(list.toAgonesList(), updateMask, StreamObserverUtil.listStoreToAgonesList(observer));
    }
    ...
    public void updateCounter(CounterStore counter, FieldMask updateMask, StreamObserver<CounterStore> observer) {
        this.alphaSdk.updateCounter(counter.toAgonesCounter(), updateMask, StreamObserverUtil.counterStoreToAgonesList(observer));
    }
    ...
    public void setPlayerCapacity(long capacity, VoidStreamObserver observer) {
        this.alphaSdk.setPlayerCapacity(Count.newBuilder().setCount(capacity).build(), observer.toAlpha());
    }
    ...
    public static final String AGONES_SDK_GRPC_HOST = Objects.requireNonNullElse(System.getenv("AGONES_SDK_GRPC_HOST"), "localhost");
    ...
    private static int readPortFromEnv(String key, int defaultValue) {
        ...
        try {
            ...
            throw new NumberFormatException("Cannot parse '%s' as port from environment variable '%s'".formatted(value, key));
        }
    }
    ...
    public static AgonesClient create(String address) {
        return new AgonesClient(
                ManagedChannelBuilder.forAddress(address, AGONES_SDK_GRPC_PORT)
                        .usePlaintext()
                        .build(),
                AGONES_SDK_TIMEOUT
        );
    ...
    public static AgonesClient create(int port) {
        return new AgonesClient(
                ManagedChannelBuilder.forAddress(AGONES_SDK_GRPC_HOST, port)
                        .usePlaintext()
                        .build(),
                AGONES_SDK_TIMEOUT
        );
