...
public class StreamObserverUtil {
  ...
  public static StreamObserver<PlayerIDList> listToAlphaSdkIdList(
      StreamObserver<List<String>> observer) {
    return new TransformedStreamObserver<>(idList -> List.copyOf(idList.getListList()), observer);
  }

  public static StreamObserver<dev.agones.sdk.alpha.List> listStoreToAgonesList(
      StreamObserver<ListStore> observer) {
    return new TransformedStreamObserver<>(ListStore::new, observer);
  }

  public static StreamObserver<Counter> counterStoreToAgonesList(
      StreamObserver<CounterStore> observer) {
